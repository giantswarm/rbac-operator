...
func Test_AutomationSA(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			ExpectedSAs: []*corev1.ServiceAccount{
				clustertest.NewServiceAccount(pkgkey.AutomationServiceAccountName, pkgkey.DefaultNamespaceName),
			},
			ExpectedRoleBindings: []*rbacv1.RoleBinding{
				clustertest.NewRoleBinding(
					pkgkey.WriteAllAutomationSARoleBindingName(),
					pkgkey.DefaultNamespaceName,
					clustertest.NewSingletonSASubjects(pkgkey.AutomationServiceAccountName, pkgkey.DefaultNamespaceName),
				),
			},
			ExpectedClusterRoleBindings: []*rbacv1.ClusterRoleBinding{
		...
		},
		{
			...
			InitialObjects: []runtime.Object{
				clustertest.NewServiceAccount(pkgkey.AutomationServiceAccountName, pkgkey.DefaultNamespaceName),
				clustertest.NewRoleBinding(pkgkey.WriteAllAutomationSARoleBindingName(), pkgkey.DefaultNamespaceName, []rbacv1.Subject{}),
				clustertest.NewClusterRoleBinding(pkgkey.ReadAllAutomationSAClusterRoleBindingName(), []rbacv1.Subject{}),
				clustertest.NewClusterRoleBinding(pkgkey.WriteOrganizationsAutomationSARoleBindingName(), []rbacv1.Subject{}),
				clustertest.NewClusterRoleBinding(pkgkey.WriteClustersAutomationSARoleBindingName(), []rbacv1.Subject{}),
				clustertest.NewClusterRoleBinding(pkgkey.WriteNodePoolsAutomationSARoleBindingName(), []rbacv1.Subject{}),
				clustertest.NewClusterRoleBinding(pkgkey.WriteClientCertsAutomationSARoleBindingName(), []rbacv1.Subject{}),
				clustertest.NewClusterRoleBinding(pkgkey.WriteSilencesAutomationSARoleBindingName(), []rbacv1.Subject{}),
			},
			ExpectedSAs: []*corev1.ServiceAccount{
				clustertest.NewServiceAccount(pkgkey.AutomationServiceAccountName, pkgkey.DefaultNamespaceName),
			},
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			namespace := clustertest.NewDefaultNamespace()
			...
			clustertest.ServiceAccountsShouldEqual(t, tc.ExpectedSAs, serviceAccountList.Items)
